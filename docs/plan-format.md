...
In `tao show --json` (`tao.show.v1`), `abandonment.reason` contains the complete reason with whitespace and control characters normalized to single spaces, or `No abandonment reason was recorded.` when empty. Human-readable views retain a 96-rune excerpt; neither projection changes durable evidence.
...
Final-verification code repair has a fixed lifetime cap of two generated attempts per plan. Each generated repair slice has a non-null `verification_repair` binding containing the failed verification's `command`, `head_sha`, and `fingerprint`. An ordinary run's automatic driver may write `verification_repair_created` events as well as explicit `tao run --repair-verification`, with no schema change. The attempt count is the number of all such slices in `slices.json`, including completed attempts; it does not reset when failure evidence changes. Reaching two permanently disables further generated verification-repair slices for that plan. The cap is repository policy, not configuration.
