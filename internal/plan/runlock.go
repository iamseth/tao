...
var (
	// ErrRunLocked identifies contention with another plan driver.
	ErrRunLocked           = errors.New("plan run lock held")
...
func ReadRunLock(planDir string) (RunLock, error) {
	...
	if metadata.readPIDErr != nil {
		return RunLock{}, metadata.readPIDErr
	}
	return RunLock{PID: metadata.readPID, ProcessAlive: planRunLockProcessLive(metadata.readPID)}, nil
}

// SetRunLockSettingsForTest overrides the shared lock timeout and liveness probe
// and returns a restore function. It must not be used by parallel tests.
func SetRunLockSettingsForTest(timeout time.Duration, live func(int) bool) func() {
	oldTimeout, oldLive := planRunLockTimeout, planRunLockProcessLive
	planRunLockTimeout, planRunLockProcessLive = timeout, live
	return func() {
		planRunLockTimeout, planRunLockProcessLive = oldTimeout, oldLive
	}
}
...
// RunLockRequest identifies one plan whose ordinary run ownership must be
...
// RunLocks owns a set of ordinary plan run locks. Callers must release it.
type RunLocks struct {
	locks []*planRunLock
}

// AcquireRunLocks reserves plans in stable plan-ID order. If any acquisition
...
func AcquireRunLocks(requests []RunLockRequest, timestamp time.Time) (*RunLocks, error) {
	ordered := append([]RunLockRequest(nil), requests...)
	slices.SortFunc(ordered, func(a, b RunLockRequest) int {
	...
	for _, request := range ordered {
		lock, err := AcquireRunLock(request.PlanDir, request.PlanID, timestamp)
		if err != nil {
...
type planRunLockMetadata struct {
	...
	// Readers historically use the first PID field, while stale takeover uses
	// the last parseable PID. Preserve both interpretations of malformed locks.
	readPID    int
	readPIDErr error
}
...
// WithRunLock runs operation while holding the ordinary per-plan driver
...
// returns.
func WithRunLock(ctx context.Context, detail *PlanDetail, timestamp time.Time, operation func(context.Context) error) (err error) {
	if detail == nil {
	...
	lock, err := AcquireRunLock(detail.Dir, detail.State.Plan.ID, timestamp)
	if err != nil {
		return err
	}
	defer func() {
...
// AcquireRunLock reserves a plan until the returned handle is released.
func AcquireRunLock(planDir string, planID string, timestamp time.Time) (*planRunLock, error) {
	path, err := planRunLockPath(planDir)
	if err != nil {
		return nil, err
	}
...
func planRunLockPath(planDir string) (string, error) {
	...
	return filepath.Join(abs, runLockFileName), nil
}
...
func parsePlanRunLockMetadata(content []byte) planRunLockMetadata {
	metadata := planRunLockMetadata{readPIDErr: errors.New("parse plan run lock: pid is missing")}
	seenPID := false
	for line := range strings.SplitSeq(string(content), "\n") {
		...
		case "pid":
			...
			if !seenPID {
				seenPID = true
				metadata.readPID = pid
				metadata.readPIDErr = nil
				if err != nil || pid <= 0 {
					metadata.readPIDErr = fmt.Errorf("parse plan run lock pid %q", strings.TrimSpace(value))
				}
			}
			if err == nil {
				metadata.PID = pid
