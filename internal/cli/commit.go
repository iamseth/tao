...
var commitCommand = commandMetadata{
	name:                  "commit",
	usageLines:            []string{"commit --context [--push] [--repo-root DIR]", "commit --proposal-file FILE [--push] [--repo-root DIR]", "commit --message MESSAGE [--push] [--repo-root DIR]"},
	completionDescription: "Safely commit outside managed plan worktrees",
	long:                  "Build drift-bound standalone commit context or finalize a centrally validated commit. Context generation is read-only, including with --push. Explicit --push requires a configured upstream and publishes only the newly created commit without force; a push failure preserves the local commit. Finalization rechecks the repository snapshot, stages only allowed paths, and lets Tao create the commit. Active Tao-managed plan worktrees refuse standalone context and commits; follow the reported plan continuation, safe restart, completion-recovery, verification-repair, or rework path instead.",
...
func registerCommitFlags(fs *flag.FlagSet) {
	...
	fs.Bool("push", false, "preflight the configured upstream and push the new commit without force on finalization")
...
func (a App) commit(ctx context.Context, args []string) error {
	...
	if err := requireNoArgs(positional, "usage: tao commit (--context | --proposal-file FILE | --message MESSAGE) [--push] [--repo-root DIR]"); err != nil {
		return err
	}
	contextMode := flagBoolValue(fs, "context")
	options := taocommit.StandaloneOptions{Push: flagBoolValue(fs, "push")}
	...
	if modes != 1 {
		return errors.New("usage: tao commit (--context | --proposal-file FILE | --message MESSAGE) [--push] [--repo-root DIR]")
	}
	...
	if contextMode {
		var push *taocommit.StandalonePush
		if options.Push {
			target, err := taocommit.PreflightStandalonePush(ctx, git)
			if err != nil {
				return err
			}
			push = &target
		...
		if err := encoder.Encode(struct {
			...
			Push *taocommit.StandalonePush `json:"push,omitempty"`
		}{commitContext, push}); err != nil {
			return fmt.Errorf("write standalone commit context: %w", err)
		}
		return nil
	}

	var result taocommit.StandaloneResult
	if proposalFile != "" {
		result, err = taocommit.FinalizeStandaloneProposalWithOptions(ctx, git, repoRoot, proposal, options)
	} else {
		result, err = taocommit.FinalizeStandaloneMessageWithOptions(ctx, git, repoRoot, message, options)
	}
	...
	if result.Pushed != nil {
		return writef(a.Out, "Created local commit %s %s.\nPushed commit %s to %s.\n", abbreviateCommitSHA(result.SHA), result.Subject, result.SHA, result.Pushed.Destination)
	}
