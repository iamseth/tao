...
func TestRunWarnsOnceForStaleManagedPromptsAndExecutesCommand(t *testing.T) {
	...
	app := App{
		...
		PromptFreshnessCheck: func() ([]promptinstall.Result, error) {
			return []promptinstall.Result{
				...
				{Agent: runtimeconfig.AgentPi, Status: "stale"},
				{Agent: runtimeconfig.AgentKind("other"), Status: "unmanaged"},
				{Agent: runtimeconfig.AgentClaude, Status: "stale"},
			}, nil
		},
	}
	...
	want := "warning: stale Tao-managed prompts for pi, claude; run tao install-prompts\n"
...
func TestInstallPromptsAndDoctorManageEveryInstalledAgent(t *testing.T) {
	...
	t.Setenv("HOME", home)
	t.Setenv("TAO_AGENT", "claude")
	setPathExecutables(t, "claude", "pi")
	var out bytes.Buffer
	app := App{Out: &out, Err: &out}
	...
	claudeOutput := "[claude] installed " + filepath.Join(home, ".claude", "commands", "tao-plan.md")
	if piIndex, claudeIndex := strings.Index(text, piOutput), strings.Index(text, claudeOutput); piIndex < 0 || claudeIndex < 0 || piIndex >= claudeIndex {
		t.Fatalf("expected Pi then Claude install output independent of TAO_AGENT, got %q", text)
	}
	...
	for _, want := range []string{"[pi] current ", "[claude] current "} {
		if !strings.Contains(out.String(), want) {
	...
	text = stripANSIGreen(out.String())
	for _, want := range []string{"selected runtime agent: claude", "prompts (pi):", "prompts (claude):", "✓ ok      pi (pi)", "✓ ok      claude (claude)"} {
		if !strings.Contains(text, want) {
			t.Fatalf("expected %q in doctor output, got %q", want, text)
		}
	}
	if strings.Index(text, "prompts (pi):") >= strings.Index(text, "prompts (claude):") {
	...
	compact := stripANSIGreen(out.String())
	if !strings.HasPrefix(compact, "agents: pi, claude\n") {
