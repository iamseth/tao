...
func TestZshCompletionScriptCompletesMergePlanIDs(t *testing.T) {
	...
	for _, want := range []string{"--all[merge every reviewed and approved plan in one atomic batch]", "--auto-eject[automatically eject an attributed non-converging plan and reland the rest]", "--dry-run[preview batch candidates and order without durable changes]", "--restart[discard safe pre-landing recovery state and start again: a stale single-plan merge intent, or batch recovery state with --all]", "--force[bypass pre-merge approval, review-base, and dirty-worktree gates, not conflict-resolution safety or independent review (single-plan only)]", "--record-only[record an external merge and run cleanup (single-plan only)]", "--no-squash[preserve plan commits with rebase-plus-fast-forward; conflicts remain manual (single-plan only)]", "--no-verify[skip post-merge command verification, not structural validation or independent review (single-plan only)]", "--verify-command[override the post-merge build/test verification command]:command:", "$(${words[1]} complete plan-ids 2>/dev/null)"} {
