...
func TestUICommandRegistrationAndHelp(t *testing.T) {
	...
	for _, want := range []string{"keyboard-driven dashboard", "Notes, Plans, Settings, and Debug tabs", "Plans is the initial tab", "Tab or the right arrow", "Shift+Tab or the left arrow", "Repository focus is shared by Plans and Notes", "immediate work under NOW, planned work under NEXT, and terminal plans under HISTORY", "immediate operational actions such as MONITOR, APPROVE, or MERGE", "Operational urgency always takes precedence", "disposition, valid sequence relationships, categorical priority, and recent activity", "missing, duplicate, or cyclic relationships only warn", "legacy plans remain visible as unranked", "RUN AGE is elapsed time for an observed invocation", "NEXT is a derived advisory label", "Press Enter to inspect the selected plan's full decision and lifecycle context", "Completed and abandoned plans are hidden initially", "h reveals or hides up to 15 plans in history", "m confirms a selected reviewed-plan merge", "M confirms a repository-scoped merge --all", "press Enter for the full read-only slice page", "repository-owned open notes", "full read-only detail", "per-repository pull-request defaults", "explicit true, explicit false, and inherited", "doctor problems", "resolved runtime defaults from tao status", "Plan actions and the history toggle do not act on Notes, Settings, or Debug", "q and Ctrl-C quit globally", "Esc twice within one second", "--interval", "--completed-window", "tao monitor --once", "Usage:\n  tao ui"} {
